...
class Template
{
	...
	public static function set_message($msg = '', $type = 'info')
    {
        if (empty($msg)) {
            return;
        }
        ...
        if (isset(self::$ci->session)) {
            self::$ci->session->set_flashdata('tmessage', "{$type}::{$msg}");
        }

        self::$tMessage = array('type' => $type, 'tmessage' => $msg);
    }

	public static function message($msg = '', $type = 'information')
    {
        // Does session data exist?
        if (empty($msg) && class_exists('CI_Session', false)
        ) {
            $msg = self::$ci->session->flashdata('tmessage');
            if (! empty($msg)) {
        ...
        $template = str_replace(
            ...
            array($type, $arr_icon[$type], $arr_title[$type], $message),
